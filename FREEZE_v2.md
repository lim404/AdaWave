...
## Amendment 2026-07-19 (numerical guard only, no behavioural change)
The unit-chord normalisation guard in the anisotropic weights changed
from the absolute constant `1e-12` to the scale-adaptive `1e-12 * ell_g`
(median point spacing). Purpose: exact scale equivariance of the weight
factor (the absolute constant broke exactness at extreme coordinate
scales). The guard is active only for numerically coincident points; on
all evaluation data the outputs are unchanged (end-to-end deviation
~1e-11 relative, below the PCG tolerance 1e-6). Frozen results,
parameters, and gates are untouched. Release tests: 5/5 pass.
